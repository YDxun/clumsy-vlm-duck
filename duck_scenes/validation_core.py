...
def validate_success(results,m,tasks,scene_name):
    ...
    if scene_name=="duck_workspace_v1":
        ...
        d,ev=fresh("come_to_owner");owner=np.asarray(d.xpos[bid(m,"person_owner")],float);set_pose(m,d,"trunk_base",[owner[0]-.4,owner[1],.125]);mujoco.mj_forward(m,d);a,_,_=ev.step(.40);b,_,_=ev.step(.40);set_pose(m,d,"trunk_base",[owner[0]-.6,owner[1],.125]);mujoco.mj_forward(m,d);c,_,_=ev.step(.10);checks["come_to_owner"]=not a and b and not c
        checks["retrieve_ball"]=body_in_zone("retrieve_ball","ball","zone_green")
        d,ev=fresh("follow_owner");owner=np.asarray(d.xpos[bid(m,"person_owner")],float);set_pose(m,d,"trunk_base",[owner[0]-.7,owner[1],.125]);d.qvel[:]=0;mujoco.mj_forward(m,d);a,_,_=ev.step(2.5);b,_,_=ev.step(1.2);checks["follow_owner"]=not a and b
...
def validate_scene(results,m,name):
    ...
    if name=="duck_workspace_v1":
        ...
        floor=np.asarray(m.geom_size[gid(m,"floor")],float)[:2]*2;dl,dr=gid(m,"geom_door_left"),gid(m,"geom_door_right");width=abs(float((d.geom_xpos[dl,1]-m.geom_size[dl,1])-(d.geom_xpos[dr,1]+m.geom_size[dr,1])));people=[]
        for b in ("person_owner","person_stranger"):
            i=bid(m,b);gs=[j for j in range(m.ngeom) if int(m.geom_bodyid[j])==i];people.append({"body":b,"bottom_z":min(geom_min_z(m,d,j) for j in gs),"rgba":np.asarray(m.geom_rgba[gs[0]],float).tolist()})
        ok=va(floor,[6,5],1e-6) and width>=.5 and all(abs(p["bottom_z"])<1e-4 for p in people) and not va(people[0]["rgba"][:3],people[1]["rgba"][:3],.2);results.add("HOME_LAYOUT",ok,"6x5 room, measured door clearance, grounded/distinct people",floor_size=floor.tolist(),door_clear_width_m=width,people=people)
