...
class DuckVlmLoop:
    ...
    # Skills that are allowed to keep running (and to be chosen) while the duck is
    # down. Without this the fallen branch used to cancel everything and return, so
    # STAND_UP could never even be selected.
    RECOVERY_TOKENS = ("STAND_UP",)
    ...
    def tick(self, sim_time: float, dt: float = 0.02) -> np.ndarray:
        ...
        if state.fallen:
            # A recovery skill already underway must not be cancelled.
            if self.interpreter.busy and (self.interpreter.token or "") in self.RECOVERY_TOKENS:
            ...
            self.interpreter.cancel(reset_action=True)
            # A finished decision still has to start, otherwise the model can never
            # pick STAND_UP while the duck is on the ground.
            started = self._poll_pending(state, sim_time)
            if started is not None:
                return started
            # The simulator reset is a safety net, not a solution: give the model the
            # grace window first so a real recovery can happen.
            if (self.loop_config.plugins.fall_recovery
                    and sim_time - self._fall_since >= max(self.loop_config.fall_reset_delay_s,
                                                           self.loop_config.recovery_grace_s)):
                self.reset_callback()
                self._fall_since = None
                self.last_result = "fall_recovered"
                self.recorder.record_event({"type": "fall_recovery", "sim_time": sim_time})
                return zero
            if not (self.paused and not self.single_step) and self._pending is None:
    ...
    def _poll_pending(self, state: DuckState, sim_time: float) -> np.ndarray | None:
        """If a VLM decision has finished, log it and start the chosen action.

        Shared by the normal path and the fallen path: a duck on the ground still
        needs its chosen token (STAND_UP) to actually start.
        """
        zero = np.zeros(3, dtype=np.float32)
