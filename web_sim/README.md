...
### 技能 token（翻滚 / 跳舞 / 踢球 / 坐 / 起身）

技能的实现方式：**热切换 ONNX 策略**。所有策略共用同一套 61 维观测契约，
所以任意时刻都能换人接管，不用重置仿真。一个技能 token = 切到自己的策略 →
跑固定时长 → 自动切回 `alpha_walking`。

策略清单在 `web_sim/policies.json`，文件本身不进仓库；缺哪个就跳过哪个。

**只有实测有效的技能才会进 VLM 的候选词表**（`actions.js` 的 `verified` 标记 +
`availableTokens()`）。这一条是照 Python `available_tokens()` 的原则来的：
发给模型一个它做不到的动作，只会让它学会输出一个没反应的词。

| token | 策略 | 实测结果 | 进词表 |
| --- | --- | --- | --- |
| `ROLL` | `roulade.onnx` | **能翻滚**：upright 1 → **-0.95**（完全翻过去）→ 1.00，2.8 s | ✅ |
| `DANCE` | `happy_hop.onnx` | 会下蹲/起跳（upright 掉到 0.46） | ✅ |
| `KICK_L/R` | `ball_kick_left/right.onnx` | **原地看不出动作**：扫过 0.10–0.25 m × ±0.05 m 共 15 个站位，球一动不动 | ❌ |
| `SIT` | `alpha_sitstand.onnx` | 从站立跑 5 s 没有可见动作 | ❌ |
| `STAND_UP` | `alpha_standup.onnx` | 从人为放倒的姿态能抬到 upright 0.86，但同样条件下普通站立策略也能回到 1.00 —— 说明我的测试姿态不成立，需要正规的摔倒注入才能验 | ❌ |

九个策略文件的 SHA256 各不相同（不是同一份模型换了个名字），踢球那两个还和
`duck_scenes/policies/README.md` 里登记的官方哈希一致。

未验证的技能仍然**摆在手动面板上**（可以自己试），并在界面上标了原因；
要让踢球真正可用，下一步是把 `_dev_cockpit/local_kick.py` 那套
「站稳 → 站位 → 进动作 → 恢复」状态机搬过来 —— 那是独立的一步。

### 关于"翻滚翻不过去"

不是策略的问题：`roulade.onnx` 在浏览器仿真里实测能完整翻过去并站起来。
之前翻不了是因为**网页端根本没有 ROLL 这个 token**（词表里没有、解释器也不认），
现在点「翻滚」按钮或让 VLM 输出 `ROLL` 都能翻。

## 十、踢球站位（做到哪一步了，如实记录）

### 根因：踢球策略**故意看不见球**

`microduck_rl/.../microduck_ball_kick_env_cfg.py` 里写得很直白：

> The policy is **BLIND to the ball** (no ball obs in the actor): the real robot has no ball
> sensing — **the operator aims the robot at the ball**. Robustness to placement error comes
> from ±2 cm ball-position DR at reset.

球必须被摆在机器人偏航系的 **(x=0.09, |y|=0.042)**（右脚在 -y），容差 **±0.015 m**。
所以"精确站位"不是锦上添花，是**使用这个策略的前提**。而 VLM 的步子一步 0.07 m，
永远不可能对进 1.5 cm —— 这一段只能交给规则闭环，这正是 `src/station.js` 干的事。

### 两件只有量了才知道的事

1. **站位精确时踢得非常准**：静止站在训练站位上踢，球飞 **1.14 m**，
   3 次完全一致；方向**始终是鸭头朝向 -33°**（右脚）。之前 15 个站位全踢空，
   是因为差在几厘米上就完全踢不到 —— 这个策略没有"差不多"。
2. **最后 20 cm 必须靠航位推算**：站位点上球只在相机前方 9 cm，早就掉出画面下沿了
   （实测 <0.14 m 就出画）。如果坚持"看见才算"，最需要精度的最后一段反而在瞎走 ——
   这大概率是以前踢不准的另一半原因。所以 `StationKicker` 见到球就把计划**锁存**，
   之后靠推算走完，不再读任何真值。

### 实现（`src/station.js` + agent 的规则接管）

```
预备点（球背向目标一侧 0.45 m）→ 站位点（容差 2.5 cm）→ 对准（容差 4.6°）→ 站定 → 起脚
```

绕到侧后方再进站位点，是为了不把球撞飞（正面走过去必然撞上）。
站位阶段由规则**完全接管**，不经过 VLM；球一进区域就宣告完成，没进就重新站位再踢。
还有一层**瞄准自标定**：每踢一脚，用球的真实飞行方向反推偏角，下一脚补偿。

### 实测状态：**跑通了** ✅

| 检查 | 结果 |
| --- | --- |
| 站位精度（计划 (0.851, 0.0865)） | **实到 (0.86, 0.08)**，误差 ~1 cm ✅ |
| 朝向精度（计划 -35.6°） | 实到 **-32°** ✅ |
| 静止定点起脚 | 球飞 **1.14 m**，3/3 一致 ✅ |
| **完整任务跑（`kick_ball_to_zone`）** | 球 (0.90,0) → **(1.45,-1.10)**，距绿区圆心 **0.11 m**（半径 0.35）→ **进区** ✅ |

```
球总位移 1.23 m ｜ 1 脚 ｜ 1372 控制步 ｜ 直立度 1.00
```

### 它为什么之前不稳定，以及三处修法

同一套站位，之前单脚的结果在 **1.4 m / 0.085 m / 完全没碰到** 之间跳。三个原因，
都是量出来的：

1. **"慢速接近"在这个走路策略下不存在**：步态地板 vx=0.22，我写的 0.06~0.20 m/s
   全被 `normalizeTwist` 抬到 0.22 —— "慢慢挪"实际是"用 0.22 撞过去"。
   → 改成**脉冲驱动**：单拍给足 0.24，靠通断比压平均速度。
2. **起脚必须在真正静止时发生**：原来只数 20 拍，鸭子往往还带着接近时的速度。
   → 改成**运动量门限**：`|v| < 0.06 m/s 且 |ω| < 0.45 rad/s` 才允许起脚
   （训练也是从稳定站立起步的）。
3. **踢空就没有标定，于是永远踢不准**：偏角要用"上一脚球的落点"反推。
   → 加**站位抖动搜索**：踢完先看球动没动，没动就把站位点前后/左右挪 2 cm 再来，
...
**`push_red_cube` 完整跑通**：方块从 (0.70,-0.65) 被推到 (-0.93,-1.35)，
距蓝区圆心 **0.212 m**（半径 0.30）→ **进区**，控制器自己宣告"任务完成"。

```
方块总位移 1.771 m ｜ 7796 个控制步 ｜ 墙钟 65 s ｜ 直立度 1.00 ｜ 全程零横向漂移
```

这是这套系统**第一个端到端完成的场景任务**（按场景包自己的成功判据）。

推比踢宽松（不需要 1.5 cm 容差，站位点留余量、走进去顶住就行），所以 `Pusher` 复用
了同一套几何：算"物体背向目标一侧"的预备点/接触点 → 走过去对准 → 轻推 → 每推几轮
**后退到看得见的地方重新观测**（棘轮式，因为推的时候物体就在脚前、早出画了）。

**接触距离是量出来的**（把方块固定在 (0.70,-0.65)，鸭子摆在方块后方不同距离，
连续驱动 1.8 s，看方块被推走多远）：

| 站位距离 | 方块位移 | 沿目标方向 | 横向漂移 |
| --- | --- | --- | --- |
| 0.20 m | **0**（脚尖够不着） | 0 | 0 |
| 0.16 m | 0.024 m | 0.023 | ~0 |
| 0.13 m | 0.058 m | 0.058 | 0 |
| 0.10 m | 0.088 m | 0.088 | 0 |
| 0.07 m | 0.122 m | 0.122 | 0 |
| 0.04 m | 0.141 m | 0.140 | 0 |

**方向几乎完美**：每一行的"沿目标方向"都等于总位移，横向漂移为 0 ——
也就是说**只要推得到，方块就沿着站位算出来的直线走**，这一环没有问题。

原来的参数 `contactStandoffM=0.13, contactTolM=0.035` 最远允许站到 0.165 m ——
**刚好落在够不着的区间**，这就是之前方块一动不动的原因。现在改成 0.07 ± 0.02 m，
并把连续推改成"推 25 拍 → 退回来重看一眼"的短促脉冲。

完整任务实测：

| 用例 | 结果 |
| --- | --- |
| 推球（15 g 圆球，目标 1.23 m 外） | **推不了**：见下 |
| 推方块（`push_red_cube`，1.63 m 外） | 站位走对了（到接触点 0.85,-0.62），但当时参数落在够不着的区间，**方块没动**；参数已按上表修正，尚未复跑确认 |
| 定点接触实验 | 方块**确实推得动**，且方向完全正确（见上表） ✅ |

两个已定位的问题：

1. **球是圆的**：连续推进会把球推飞（实测一次推出 1.9 m，直接滚过界）。改成"走一拍停两拍"
   的轻推之后不再飞，但仍然不收敛到区域内 —— 圆球靠推做精确落点本来就难，
   需要更短的脉冲 + 每个脉冲后重新观测。
### 推球踩过的四个坑（都修好了）

1. **接触距离猜错了**：原来 0.13±0.035 m，最远允许站到 0.165 m —— 刚好够不着。
   实测表在上面，改成 0.07±0.02。
2. **没有绕行点**：从起点直着去站位点会**穿过物体**，实测鸭子穿过方块跑到另一侧、
   然后朝反方向推了 0.45 m。加了侧向 0.45 m 的绕行点，挑离鸭子更近的一侧绕。
3. **兜底搜索只会往一个方向转**：方块在右前方 -43° 时，只会左转的策略要扫 317°
   （实测整整 7 秒才看见）。加了 `alternate_search` 规则（每 6 次决策换方向），
   最坏变成 137°，搜索时间从 7 s 降到约 2 s。
4. **"后退观测"半路被打断**：后退还没走完就来了一次观测，触发重算后直接从半路
   切去接触点，于是从错误的一侧穿过方块（实测把方块推到反方向 0.8 m）。
   现在重算一律回到第一个航点。

另外把整任务跑的耗时从 5~10 分钟压到 **65 秒**：观测时那次"把屏幕恢复成用户视角"
的多余渲染，在批量跑里可以关掉（`view.restoreAfterCapture = false`）。

### 圆球：结论是"不该用推的"

球是 15 g 的球体，走路时脚一接触就是一次冲量，**根本推不慢**：

| 推进脉冲 | 球被送出 | 说明 |
| --- | --- | --- |
| 90 拍（鸭子前进 0.25 m） | **1.9 m** | 放大 7.6 倍，直接滚过界还在场地里弹 |
| 25 拍（鸭子前进 0.05 m） | **1.4 m** | 还是踢飞的量级 |

试过按物体类型区分脉冲长度（球 25 拍 / 方块 90 拍），球依然飞出去 —— 因为
**接触冲量和"推多久"无关**，第一次接触就决定了球速。而走路策略有个 0.22 m/s 的
步态地板，没法用"更慢的姿势"去接触。

所以结论很明确：**球该走踢球策略那条路**（那个策略就是为球设计的，目标球速
1.0 m/s、飞行 1.14 m，可控），推球这条路只适合方块/杯子这类**不滚**的物体 ——
而方块那条已经跑通了。
