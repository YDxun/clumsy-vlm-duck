...
| 完整任务跑（`kick_ball_to_zone`） | 4 次都没进球区（离圆心 0.74~1.23 m）❌ |

完整任务里单脚的结果在 **1.4 m / 0.085 m / 完全没碰到** 之间跳。

### 已经定位到的三个原因（下一步就照这个改）

1. **"慢速接近"在这个走路策略下不存在**：实测步态地板 vx=0.22，我写的
   0.06~0.20 m/s 全被 `normalizeTwist` 抬到 0.22 —— 于是我精心设计的"慢慢挪"
   实际是"用 0.22 撞过去"，near-target 时必然冲过站位点。
   正确做法是**脉冲驱动**（单拍给 0.24，靠通断比压平均速度）；我试的两组通断比
   一组太慢（3000 步走不到）、一组还是会在起脚瞬间带着速度。
2. **起脚必须在真正静止时发生**：训练是从稳定的站立姿态起步的，而我现在只用
   固定 20 拍做"站定"，没有检查实测的 |v| 和 |ω|。应该改成运动量门限。
3. **自标定需要先踢中一次才能起步**：偏角实测从静止的 -33° 变到走完一整套后的 -79°，
   但偏角要用"上一脚球的落点"才能反推 —— 踢空就没有观测，标定启动不了。
...
| 用例 | 结果 |
| --- | --- |
| 推球（15 g 圆球，目标 1.23 m 外） | **推动了**：球从 (0.90,0) 被推到 (1.32,-1.76)，大致朝着区域 |
