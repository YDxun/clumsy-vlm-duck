...
1. **球是圆的**：连续推进会把球推飞（实测一次推出 1.9 m，直接滚过界）。改成"走一拍停两拍"
   ...
   需要更短的脉冲 + 每个脉冲后重新观测。
2. **后退观测的目标点会失效**：物体位置是锁存的，一旦过期，后退点可能算到几米外，
   控制器就朝那儿跑。已加安全阀（后退点 >1.2 m 就丢弃计划、等下一次观测），
   但"什么时候该重看"还需要按实测的漂移量来调。

结论：**推的框架和方向都是对的，接触距离这个关键参数也量出来了**，
但"把物体稳定送进区域"的完整复跑还没拿到。下一步就是把改完参数的
`push_red_cube` 复跑确认（方块不滚、方向不漂，是最有希望跑通的一条）。
