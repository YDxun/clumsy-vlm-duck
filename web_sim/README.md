...
| 推方块（`push_red_cube`，1.63 m 外） | 站位走对了（到接触点 0.85,-0.62），但**方块没动**，随后控制器跑到 3 m 外的错误后退点 |
...
结论：**推的框架是对的、轻物体确实能推动，但"把物体稳定送进区域"还没做到**。
下一步优先级：先修 `push_red_cube`（方块不滚、接触后稳定），再回头调圆球。
