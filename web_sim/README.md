...
用户还提了"推东西也尝试做到"，这一轮没有落地。好消息是**推比踢简单**：
不需要 1.5 cm 的精度，站位点可以从需要的方向留出余量，然后直接走进物体把它顶走。
`planStation()` 算出的"背向目标一侧的站位点"和 `StationKicker` 的航点/对准逻辑可以
直接复用，把最后的"起脚"换成"持续前进"即可 —— 这是下一个增量。
